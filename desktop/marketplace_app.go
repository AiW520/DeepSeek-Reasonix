...
var reviewedMarketplace = []MarketplaceEntry{
	...
	{ID: "skill-video-shotcraft", Kind: "skill", Name: "Video Shotcraft", Description: "Plan and execute polished video shot lists, coverage, and production workflows.", Category: "video", Repository: "https://github.com/Vincentwei1021/video-shotcraft", Commit: "0d6f0b57f0d4d6700761644c07f7ef03c3e50234", License: "Apache-2.0", Author: "Vincentwei1021", Capabilities: []string{"skill"}, Risk: "low"},
	{ID: "skill-video-kit", Kind: "skill", Name: "Claude Video Kit", Description: "A structured assistant workflow for scripting, editing and delivering video projects.", Category: "video", Repository: "https://github.com/runesleo/claude-video-kit", Commit: "f09790c6e90e610b9dbdec0d1983bd5abeecd0bf", License: "MIT", Author: "runesleo", Capabilities: []string{"skill"}, Risk: "low"},
	{ID: "skill-remotion-motion", Kind: "skill", Name: "Remotion Motion Graphics", Description: "Motion graphics planning and Remotion production guidance for code-driven video.", Category: "video", Repository: "https://github.com/haidrrrry/claude-remotion-skill", Commit: "1dcbe5e3fc6cf970bd10d3cc05f0a8a5d19d0383", License: "MIT", Author: "haidrrrry", Capabilities: []string{"skill"}, Risk: "low"},
}

func marketplaceInstallSource(entry MarketplaceEntry) string {
	repository := strings.TrimRight(strings.TrimSpace(entry.Repository), "/")
	if tree := strings.Index(repository, "/tree/"); tree >= 0 {
		repository = repository[:tree]
	}
	if entry.Kind == "skill" {
		return repository + "/tree/" + entry.Commit
	}
	return repository
}
...
func (a *App) PlanMarketplaceInstall(id string) (string, error) {
	...
	source := marketplaceInstallSource(entry)
