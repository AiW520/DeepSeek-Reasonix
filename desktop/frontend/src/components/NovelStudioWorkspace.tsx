...
export function NovelStudioWorkspace({ onClose, registerBeforeLeave }: Props) {
  ...
  const [newInput, setNewInput] = useState(EMPTY_INPUT);
  const [dirty, setDirty] = useState(false);
  const [memory, setMemory] = useState<NovelLongMemory | null>(null);
  const [jobs, setJobs] = useState<NovelAutoWriteJob[]>([]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const [nextMemory, nextJobs] = await Promise.all([app.NovelMemory(project.id), app.ListNovelAutoWriteJobs(project.id)]);
        if (!alive) return;
  ...
  return <section className="novel-studio" aria-label="AI 小说工作室">
    ...
    <div className="novel-studio__body">
      ...
      {project ? <main className="novel-studio__main"><div className="novel-studio__project-bar"><div><span className="novel-studio__eyebrow">{project.genre} · {project.tone}</span><h2>{project.title}</h2><p>{project.premise || "给你的故事一个清晰的起点。"}</p></div><div className="novel-studio__stats"><strong>{project.chapters.reduce((sum, c) => sum + c.wordCount, 0).toLocaleString()}</strong><small>总字数</small><strong>{project.chapters.length}</strong><small>章节</small></div></div>
        ...
        {tab === "characters" && <div className="novel-character-grid">{project.characters.map((item) => <article key={item.id}><div className="novel-character-grid__avatar">{item.name.slice(0, 1)}</div><input value={item.name} onChange={(e) => updateCharacter(item.id, { name: e.target.value })} aria-label="人物名称" /><input value={item.role} onChange={(e) => updateCharacter(item.id, { role: e.target.value })} aria-label="人物角色" /><textarea value={item.traits} onChange={(e) => updateCharacter(item.id, { traits: e.target.value })} placeholder="性格、外貌与声音" /><input value={item.arc} onChange={(e) => updateCharacter(item.id, { arc: e.target.value })} placeholder="人物弧光" /></article>)}<button type="button" className="novel-add-card" onClick={addCharacter}><Plus size={19} />添加人物卡</button></div>}
        {tab === "outline" && <div className="novel-outline"><div className="novel-outline__head"><div><span>STORY ARCHITECT</span><h3>让主 AI 规划你的长篇结构</h3></div><button type="button" onClick={() => void ai("outline")} disabled={busy}><Sparkles size={14} />生成大纲</button></div>{project.outlines.length ? project.outlines.map((item, index) => <div className="novel-outline__item" key={item.id}><b>{String(index + 1).padStart(2, "0")}</b><div><strong>{item.title}</strong><p>{item.summary}</p></div></div>) : <div className="novel-empty">还没有大纲，先让主 AI 帮你建立故事骨架。</div>}</div>}
        {tab === "review" && <div className="novel-review"><div className="novel-review__head"><div><span>EDITORIAL REVIEW</span><h3>{issues.length ? `发现 ${issues.length} 个可优化点` : "章节审查中心"}</h3></div><button type="button" onClick={() => void ai("review-final")} disabled={busy}><Check size={14} />运行完稿审查</button></div>{issues.length ? issues.map((issue, index) => <article key={`${issue.title}-${index}`} className={`novel-issue novel-issue--${issue.severity}`}><span>{issue.severity === "critical" ? "严重" : issue.severity === "warning" ? "建议" : "提示"}</span><div><strong>{issue.title}</strong><small>{issue.category}</small><p>{issue.detail}</p><em>建议：{issue.suggestion}</em></div></article>) : <div className="novel-empty">完成一段正文后运行实时审查，主 AI 会继续保持创作节奏。</div>}<div className="novel-history"><div><History size={15} />版本历史</div><button type="button" onClick={() => void loadVersions()}>加载当前章节版本</button>{versions.map((version) => <button type="button" key={version.id} onClick={() => void restoreVersion(version.id)}>{version.label} · {version.wordCount} 字</button>)}</div></div>}
      </main> : <div className="novel-empty novel-empty--full"><BookOpen size={30} /><h2>开始一部新小说</h2><p>创建项目后，世界观、人物、章节和三 AI 协作都会在这里展开。</p><button type="button" onClick={() => setShowNew(true)}><Plus size={15} />创建项目</button></div>}
    </div>
    {busy && <div className="novel-studio__busy"><Loader2 size={16} className="spin" />AI 正在处理...</div>}{message && <div className="novel-studio__toast">{message}</div>}
    {showNew && <div className="novel-modal-backdrop"><div className="novel-modal"><div className="novel-modal__head"><div><span>NEW PROJECT</span><h2>创建小说项目</h2></div><button type="button" onClick={() => setShowNew(false)}><X size={17} /></button></div><label>作品名称<input autoFocus value={newInput.title} onChange={(e) => setNewInput({ ...newInput, title: e.target.value })} placeholder="例如：雾港来信" /></label><label>类型<input value={newInput.genre} onChange={(e) => setNewInput({ ...newInput, genre: e.target.value })} /></label><label>一句话梗概<textarea value={newInput.premise} onChange={(e) => setNewInput({ ...newInput, premise: e.target.value })} placeholder="主角、目标与阻力是什么？" /></label><div className="novel-modal__actions"><button type="button" onClick={() => setShowNew(false)}>取消</button><button type="button" className="primary" onClick={() => void createProject()} disabled={!newInput.title.trim() || busy}><Plus size={15} />创建并开始</button></div></div></div>}
  </section>;
}

function NovelAutoPanel({ project, jobs, target, words, instruction, busy, onTarget, onWords, onInstruction, onStart, onControl }: { project: NovelProject; jobs: NovelAutoWriteJob[]; target: number; words: number; instruction: string; busy: boolean; onTarget: (value: number) => void; onWords: (value: number) => void; onInstruction: (value: string) => void; onStart: () => void; onControl: (action: "pause" | "resume" | "stop", id: string) => void }) {
  const active = jobs.find((item) => ["queued", "running", "paused", "failed"].includes(item.status));
  const progress = active ? Math.min(100, Math.round((active.completedChapters / Math.max(1, active.targetChapters)) * 100)) : 0;
  return <div className="novel-auto">
    <section className="novel-auto__hero"><div><span>AUTONOMOUS SERIAL WRITING</span><h3>让故事自己向前生长</h3><p>主笔、连续性编辑与完稿编辑按章协作。每章完成即保存并更新长记忆，你可以随时暂停或停止。</p></div><div className="novel-auto__engine"><i /><span>后台创作引擎</span><strong>{active?.status === "running" ? "运行中" : active?.status === "paused" ? "已暂停" : "待命"}</strong></div></section>
    {active ? <section className="novel-auto__active"><div className="novel-auto__progress-head"><div><small>{active.phase}</small><strong>{active.currentChapterTitle || "正在准备下一章"}</strong></div><b>{progress}%</b></div><div className="novel-auto__progress"><span style={{ width: `${progress}%` }} /></div><div className="novel-auto__metrics"><span><b>{active.completedChapters}</b> 已完成</span><span><b>{active.targetChapters}</b> 目标章节</span><span><b>{active.targetWordsPerChapter}</b> 单章目标字数</span></div>{active.lastError && <p className="novel-auto__error">{active.lastError}</p>}<div className="novel-auto__controls">{active.status === "running" || active.status === "queued" ? <button type="button" onClick={() => onControl("pause", active.id)}><Pause size={15} />暂停</button> : <button type="button" onClick={() => onControl("resume", active.id)}><RotateCcw size={15} />继续</button>}<button type="button" className="danger" onClick={() => onControl("stop", active.id)}><Square size={14} />停止任务</button></div></section> : <section className="novel-auto__setup"><div className="novel-auto__fields"><label>写到第几章<input type="number" min={project.chapters.length + 1} max={1000} value={target} onChange={(e) => onTarget(Number(e.target.value))} /></label><label>每章目标字数<input type="number" min={500} max={20000} step={100} value={words} onChange={(e) => onWords(Number(e.target.value))} /></label></div><label>连续创作要求<textarea value={instruction} onChange={(e) => onInstruction(e.target.value)} placeholder="例如：保持慢热悬疑节奏，每三章回收一个伏笔，不要过早揭示幕后人物。" /></label><div className="novel-auto__launch"><div><strong>从第 {project.chapters.length + 1} 章开始</strong><small>关闭工作室后仍会在桌面后台继续；退出应用后，下次启动会从章节检查点恢复。</small></div><button type="button" onClick={onStart} disabled={busy || target <= project.chapters.length}><Play size={16} />开始连续创作</button></div></section>}
    {jobs.length > 0 && <section className="novel-auto__history"><h4>任务记录</h4>{jobs.slice(0, 6).map((job) => <div key={job.id}><span className={`status status--${job.status}`} /> <strong>{job.phase}</strong><small>{job.completedChapters}/{job.targetChapters} 章 · {new Date(job.updatedAt).toLocaleString()}</small></div>)}</section>}
  </div>;
}

function NovelMemoryPanel({ memory, busy, onImport }: { memory: NovelLongMemory | null; busy: boolean; onImport: () => void }) {
  const hasBible = Boolean(memory?.bible?.filename);
  return <div className="novel-memory"><section className="novel-memory__import"><div className="novel-memory__icon"><Brain size={28} /></div><div><span>LONG-TERM STORY MEMORY</span><h3>{hasBible ? memory?.bible.filename : "上传作品圣经，建立故事的长期记忆"}</h3><p>{hasBible ? `${memory?.bible.characters.toLocaleString()} 字符 · 已提取世界规则、人物状态、时间线和创作约束` : "支持 Markdown、TXT 和 DOCX。原文件保存在本地；点击导入并分析后，内容会发送给当前配置的模型供应商进行结构化提取。"}</p></div><button type="button" onClick={onImport} disabled={busy}><Upload size={16} />{hasBible ? "重新导入" : "导入并分析"}</button></section>
    {hasBible ? <div className="novel-memory__grid"><article className="novel-memory__summary"><span>全局故事摘要</span><p>{memory?.globalSummary || memory?.bible.summary}</p><em>{memory?.bible.style}</em></article><MemoryList title="时间线" items={memory?.timeline || []} /><MemoryList title="确定事实" items={memory?.facts || []} /><MemoryList title="未回收伏笔" items={memory?.openThreads || []} /><MemoryList title="不可违背约束" items={memory?.constraints || []} /><article><span>人物最新状态</span>{Object.entries(memory?.characterStates || {}).slice(0, 12).map(([name, state]) => <p key={name}><strong>{name}</strong>{state}</p>)}</article></div> : <div className="novel-memory__empty"><FileText size={26} /><strong>还没有作品圣经</strong><span>导入后，连续创作不会只依赖最近几章，而会持续维护可检索的故事事实。</span></div>}
  </div>;
}

function MemoryList({ title, items }: { title: string; items: string[] }) { return <article><span>{title}</span>{items.slice(0, 12).map((item, index) => <p key={`${title}-${index}`}>{item}</p>)}{items.length === 0 && <small>暂无记录</small>}</article>; }

function Setting({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="novel-setting"><span>{label}</span><textarea value={value} onChange={(e) => onChange(e.target.value)} /></label>; }
